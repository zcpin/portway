...
隧道卡片菜单的「诊断连接」检查已保存配置的监听端口、SSH 认证与跳板链，以及目标 TCP 服务，并显示各项耗时、失败原因和可复制的报告。本地转发通过 SSH 检查远端目标，反向转发检查本机目标；SOCKS5 没有固定目标，会明确标记为未检查。已停止的反向隧道不会申请远端监听端口，该项需启动后验证。诊断最多运行 10 秒，不保存配置、不启停隧道，也不验证数据库账号或业务协议；关闭诊断窗口或切换实例会取消请求。
...
| `Ctrl+F` | 聚焦搜索框 |
| `Ctrl+N` | 新建隧道 |
| `Ctrl+Enter` | 启停选中的隧道；未勾选时可启停唯一的搜索结果 |
| `Ctrl+Alt+1` ～ `Ctrl+Alt+9` | 按固定顺序启停前九条收藏 |

本地转发隧道的卡片菜单可「配置打开方式」：选择 http/https 网页模板，或本机程序及其参数。网址和参数支持 `{host}`、`{port}`，会替换为本地监听地址和端口；程序参数每行一个，不需要为带空格的参数额外加引号。Windows 选择完整的 `.exe` 路径，macOS 也可填写 `.app` 路径。

配置后，卡片和托盘可以「打开服务」或「启动并打开服务」。未连接时先启动并等待 SSH 与本地监听就绪，重复点击会合并；切换实例、手动停止或等待超时会取消打开，超时本身不会停止隧道。目标服务的 TCP 可达性仍可通过「诊断连接」检查。反向转发与 SOCKS5 支持收藏和快捷启停，外部打开仅用于本地端口转发。
...
| POST | `/api/tunnels/{name}/diagnose` | 检查监听、SSH 和目标 TCP，返回各项 `ok/failed/skipped` 状态、耗时和原因；不启停隧道 |
