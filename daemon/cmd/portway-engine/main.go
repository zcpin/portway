...
// Command portway-engine 把隧道引擎编译成动态库，供 Flutter 客户端通过 dart:ffi 直接调用。
//
// 与 portway-daemon 的关系：两者是同一个引擎的两种「外壳」——
...
//
// 产物除动态库外还有一个同名头文件（portway.h），只用于 C 侧对接，
// Dart 侧不依赖它（接口约定写在 internal/embedded/export.go 的包注释里）。
package main

import "C"

import _ "github.com/byteporter/portway/internal/embedded"
